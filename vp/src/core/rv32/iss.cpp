...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////////////////////////////////////
//////////////////////              Adding the Bit Manipulation extension                //////////////////////

/////// A- Basic bit manipulation instruction : Zbb


			case Opcode::ANDN:
					regs[instr.rd()] = regs[RS1] & ~(regs[RS2]);
					break;
...
// SLO,SRO : These instructions are similar to shift-logical operations from the base spec,
//           except instead of shifting in zeros, they shift in ones.
			case Opcode::SLO:
				regs[RD] = ~( ~((uint32_t)regs[RS1]) << (regs.shamt(RS2) & 31) );

				break;

			case Opcode::SRO:
				regs[RD] = ~( ~((regs[RS1]) ) >> (regs.shamt(RS2) & 31) );
				break;


//CLZ : counts the number of 0 bits at the MSB end of the argument. That is, the number of 0 bits before  the first 1 bit
//    counting from the most significant bit. If the input is 0, the output is XLEN.  If the input is -1, the output is 0.

			case Opcode::CLZ: {
				int count = 0;
				while (((regs[RS1] << count ) >> 31 ) == 0b0 && (count <32))
					{count ++;}
				regs[instr.rd()] = count;
			} break;

//CTZ : counts the number of 0 bits at the LSB end of the argument. If the input is 0,the output is XLEN. If the input is -1, the output is 0.

			case Opcode::CTZ: {
					int count = 0;
					while ( ((( regs[RS1] >> count ) & 1) == 0b0) && (count <32) )
						{ count++;}
					regs[RD] = count;
			} break;

// PCNT : This instruction counts the number of 1 bits in a register. This operations is known as population
//        count, popcount, sideways sum, bit summation, or Hamming weight
			case Opcode::PCNT: {
				int count = 0;
				for (int index = 0 ; index < 32; index++)
					{count += ( (regs[RS1]>> index) & 0x1);}
				regs[RD] = count;
			} break;


// PACK : This instruction packs the XLEN/2-bit lower halves of rs1 and rs2 into rd, with rs1 in the lower half and rs2 in the upper half.
			case Opcode::PACK: {
				uint16_t lower_part = ( regs[RS1]  << 16 ) >> 16;
				uint32_t upper_part = regs[RS2] << 16;
				regs[instr.rd()] =  upper_part | lower_part;
			} break;
...
// We define 4 single-bit instructions sbset (set), sbclr (clear), sbinv (invert), and sbext (extract),
			case Opcode::SBSET: {
				regs[RD] = regs[RS1] | (uint32_t(1) << regs.shamt(RS2)); // regs.shamt(RS2) == regs[RS2] & 31
			} break;
			...
			case Opcode::SBEXT: {
				regs[RD] = ((regs[RS1] >> regs.shamt(RS2)) & 1 ); // regs.shamt(RS2) == regs[RS2] & 31
			} break;



// B-  bit permutation instructions : Zbp

// This instruction provides a single hardware instruction that can implement all of byte-order swap,
// bitwise reversal, short-order-swap, nibble-order swap, bitwise reversal in
// a byte, etc, all from a single hardware instruction.

		case Opcode::GREV: {
		/*		uint32_t x = regs[RS1];
				//int a = regs[RS2] & 31;
				if ((a & 1) ) { x = (((x & 0x55555555) << 1) | ((x & 0xAAAAAAAA) >> 1));  }

				if ((a & 1) ) { x = (((x & 0x55555555) << 1) | ((x & 0xAAAAAAAA) >> 1));  }
				if ((a & 2) ) { x = (((x & 0x33333333) << 2) | ((x & 0xCCCCCCCC) >> 2));  }
				if ((a & 4) ) { x = (((x & 0x0F0F0F0F) << 4) | ((x & 0xF0F0F0F0) >> 4));  }
				if ((a & 8) ) { x = (((x & 0x00FF00FF) << 8) | ((x & 0xFF00FF00) >> 8));  }
				if ((a & 16)){ x = (((x & 0x0000FFFF) << 16) | ((x & 0xFFFF0000) >> 16));}
				 regs[RD] = x;*/
			regs[RD] = regs[RS1] & regs[RS2];
		} break;

			/*
// ROL,ROR :  These instructions are similar to shift-logical operations from the base spec, except they shift in
//             the values from the opposite side of the register, in order. This is also called ‘circular shift’.

			case Opcode::ROL:
				regs[RD] = ( regs[RS1] << (regs.shamt(RS2) & 31) ) | ( regs[RS1] >> ( (32 - (regs.shamt(RS2) & 31)) & 31 )  );
			break;
